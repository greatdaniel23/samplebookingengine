...
class HomepageController {
    ...
    public function getHomepageContent() {
        try {
            ...
            $homepageData = [
                ...
                'address_zipcode' => $result['postal_code'] ?? '',
    ...
    public function updateHomepageContent($data) {
        try {
            // First, check which columns exist in the villa_info table
            $columnCheck = $this->db->query("DESCRIBE villa_info");
            $existingColumns = array_column($columnCheck->fetchAll(PDO::FETCH_ASSOC), 'Field');
            ...
            if (isset($data['zipcode']) && in_array('postal_code', $existingColumns)) {
                $updateFields[] = "postal_code = :postal_code";
                $params[':postal_code'] = $data['zipcode'];
            }
            
            // Specifications - only update if columns exist
            ...
            if (isset($data['bedrooms']) && in_array('bedrooms', $existingColumns)) {
                $updateFields[] = "bedrooms = :bedrooms";
                $params[':bedrooms'] = (int)$data['bedrooms'];
            }
            
            if (isset($data['bathrooms']) && in_array('bathrooms', $existingColumns)) {
                $updateFields[] = "bathrooms = :bathrooms";
                $params[':bathrooms'] = (int)$data['bathrooms'];
            }
            
            if (isset($data['basePrice']) && in_array('price_per_night', $existingColumns)) {
                $updateFields[] = "price_per_night = :price_per_night";
                $params[':price_per_night'] = (float)$data['basePrice'];
            }
            
            // Timing - only update if columns exist
            if (isset($data['checkIn']) && in_array('check_in_time', $existingColumns)) {
                $updateFields[] = "check_in_time = :check_in_time";
                $params[':check_in_time'] = $data['checkIn'];
            }
            
            if (isset($data['checkOut']) && in_array('check_out_time', $existingColumns)) {
                $updateFields[] = "check_out_time = :check_out_time";
                $params[':check_out_time'] = $data['checkOut'];
            }
            
            // Policies - only update if columns exist
            if (isset($data['cancellationPolicy']) && in_array('cancellation_policy', $existingColumns)) {
                $updateFields[] = "cancellation_policy = :cancellation_policy";
                $params[':cancellation_policy'] = $data['cancellationPolicy'];
            }
            
            if (isset($data['houseRules']) && in_array('house_rules', $existingColumns)) {
