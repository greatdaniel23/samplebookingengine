...
try {
    ...
    error_log("Package-Room API Error: " . $e->getMessage());
    ...
    try {
        ...
        if ($success) {
            ...
            if (!empty($input['is_default'])) {
                $stmt = $db->prepare("UPDATE package_rooms SET is_default = 0 WHERE package_id = ? AND id != ?");
...
function handlePut($db) {
    try {
        ...
        if ($success) {
            ...
            if (!empty($input['is_default'])) {
                $stmt = $db->prepare("SELECT package_id FROM package_rooms WHERE id = ?");
